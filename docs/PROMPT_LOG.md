...
Phase: Fase 11D-H48-H1 · Fix Ambiguity Conversation Mapper Governance
...
- Created authorized test at `src/features/historical-import/conversational-import/ambiguity-resolution/__tests__/ambiguityConversationMapper.test.ts` with corrected relative imports.
- Updated `vitest.config.ts` to include `src/**/__tests__/**/*.test.ts` so authorized tests are runnable.
- Updated `docs/QA_CHECKLIST.md` with H48-H1 governance hotfix section.
- Updated `docs/PROMPT_LOG.md` with this entry.
- Updated `docs/ARCHITECTURE.md` with H48-H1 governance hotfix section.
- No conversation mapper logic changed. No detection logic changed. No runtime integration. No UI changes.
- Build passed. All ESLint scopes passed. All tests pass from authorized location.
- No root tests remaining.
...
Summary:
- Documented the architecture of the Historical Import Flow Adapter in docs/HISTORICAL_IMPORT_FLOW_ADAPTER_ARCHITECTURE.md.
- Defined the separation contract: CHAT_FOUNDATION (reusable base component), HISTORICAL_IMPORT_FLOW (business-specific survey flow), and FLOW_ADAPTER (translation layer between the two).
- Defined the 27 minimum flow states, 15 contractual actions, expected inputs contract, intent normalization boundary, safety & privacy policy boundaries, and thinking behavior boundary.
- Documented the gradual roadmap/migration plan for future integration phases.
- Preserved all constraints: NO_SRC_CHANGES, NO_RUNTIME_UI_INTEGRATION, NO_FLOW_MIGRATION, NO_UI_CHANGES, NO_IMPORT_EXECUTION.
...
Phase: Fase 11D-H44-H7-H1 · Chat Foundation Avatar and Semantic Icons Hotfix

Summary:
- Removed the internal sparkles icon from the assistant avatar inside the ChatFoundationMessageBubble component, ensuring it renders strictly as a gradient circle.
- Added an info icon header to safe_details messages to provide appropriate semantic context.
- Ensured icons are strictly semantic, appearing only when meaningful (e.g. warning, error, safety, handoff, confirmation, safe_details) and avoiding decorative repetition on plain text, structured messages, avatar, and other fields.
- Aligned playground checklist in ChatFoundationVisualPlayground to reflect gradient-only assistant avatar and semantic icons requirements.
- Confirmed that the playground continues to run on port 5174 and that the normal flow is unaffected on port 5173.

Commit SHA: e9841e69f182313f1c85c8be62a3077fe1bcc88b
...
Summary:
- Isolated the Chat Foundation Visual Playground to render only when the environment variable `VITE_CHAT_FOUNDATION_PLAYGROUND` is set to `true`.
- Restored the default rendering at `http://localhost:5173` to the normal Historical Import flow (`ConversationalImportWorkspace`).
- Restored and aligned the Chat Foundation visual style to match the approved chat layout and UBITS tokens:
  - User bubble style matches `rounded-2xl px-4 py-2.5 bg-muted text-foreground max-w-2xl`.
  - Assistant plain text and structured messages render as simple text without card bubbles.
  - Assistant warning, error, safety, and confirmation messages render as Cards (`bg-muted border border-border rounded-xl p-3`).
  - Assistant avatar styled with `bg-ai-gradient text-primary-foreground shadow-sm` enclosing a sparkles UbitsIcon.
  - Thinking loader styled as inline `AILoader` component.
  - Removed all hardcoded hex values, Tailwind slate overrides, or prohibited styles.
- Restructured `chatFoundationPlaygroundMessages.ts` to include simulated user and thinking messages before every assistant reply to enforce the thinking pattern rule.
- Modified `ChatFoundationVisualPlayground.tsx` layout and elements to use UBITS tokens and show clear sandbox labels.

Commit SHA: 73b50697c5bb6e476ca95536a346e0da0f031e73
...
Phase: Fase 11D-H44-H3 · Chat Foundation Message Renderer

Summary:
- Created ChatFoundationMessageRenderer and ChatFoundationMessageBubble.
- Supported visual layout and alignment for user, assistant, and system roles.
- Supported kinds: plain_text, structured, confirmation, warning, error, safe_details, thinking, and handoff.
- Enforced behavior invariants: Assistant avatar is always visible, styling is consistent without hardcoded HEX, text-white or inline styles, and safe details are rendered appropriately without raw markdown leaks.
- Made sure not to integrate into any runtime flow, playground, or route.
...
Summary:
- Locked the Conversational Chat Foundation Architecture.
- Defined ChatShell, MessageRenderer, ThinkingBehavior, ConversationPolicy, IntentNormalization, SafetyResponsePolicy, FlowAdapter, and DomainFlow layers.
- Established invariants for chat behaviors and common responses.
- Prepared the integration contract for mockups and future business flows like historical import.
- Documented in `docs/CONVERSATIONAL_CHAT_FOUNDATION_ARCHITECTURE.md`.
...
Summary:
- Locked conversational import wizard architecture for chat-first flow.
- Configured steps for survey scope, general config, structure matching, and sandbox import.
- Documented UX principles, strictly text-based actions.

Commit SHA: PENDING_COMMIT

PHASE_11D_H41_CONVERSATIONAL_IMPORT_WIZARD_ARCHITECTURE_COMPLETE
CONVERSATIONAL_IMPORT_WIZARD_ARCHITECTURE_LOCKED
ALL_ACTIONS_BY_CHAT_TEXT_ONLY
NO_TABS
NO_SIDE_PANEL_EDITOR
NO_FORM_BASED_WIZARD
NO_TABLE_BASED_MAPPING_UI
SURVEY_SCOPE_SELECTION_BY_CHAT_DEFINED
GENERAL_CONFIGURATION_BY_CHAT_DEFINED
MATCH_REVIEW_BY_CHAT_DEFINED
AMBIGUITY_RESOLUTION_BY_CHAT_DEFINED
STRUCTURE_APPROVAL_GATE_DEFINED
RESULTS_REVIEW_BY_CHAT_DEFINED
SANDBOX_IMPORT_ONLY_DEFINED
REAL_IMPORT_NOT_AUTHORIZED
SANDBOX_RESULT_LINK_DEFINED_AS_MOCK
NO_ROUTE_CREATED
NO_SRC_CHANGES
NO_RUNTIME_UI_INTEGRATION
NO_IMPORT_EXECUTION
...
VISIBLE_UI_CHECKPOINT_NO
PHASE_11D_H42_CONVERSATIONAL_WIZARD_STATE_TYPES_READY
R1H5_DEFINED_BUT_NOT_TRIGGERED
...
Summary:
- Locked the architecture to prepare a draft from a resolved review structure.
- Created `docs/DRAFT_PREPARATION_ARCHITECTURE.md` with definitions for draft entities, readiness rules, privacy boundaries, and blockages.
- Explicitly documented that Draft Preparation is not import, not persistence, and not comparison.
- Added Draft Preparation section to `docs/ARCHITECTURE.md`.
- No code, types, UI, backend, or storage were created.

Commit SHA: PENDING_COMMIT

PHASE_11D_H35_DRAFT_PREPARATION_ARCHITECTURE_COMPLETE
DRAFT_PREPARATION_ARCHITECTURE_LOCKED
HISTORICAL_LOAD_DRAFT_DEFINED
DRAFT_IS_NOT_IMPORT
DRAFT_IS_NOT_PERSISTENCE
DRAFT_IS_NOT_COMPARISON
DRAFT_INPUTS_DEFINED
RESOLVED_VIEW_AS_DRAFT_SOURCE_DEFINED
DRAFT_ENTITIES_DEFINED
DRAFT_STATUS_MODEL_DEFINED
DRAFT_READINESS_RULES_DEFINED
DRAFT_READINESS_BLOCKERS_DEFINED
DRAFT_PREVIEW_FUTURE_DEFINED
PREPARE_DRAFT_ACTION_GATE_DEFINED_BUT_NOT_BUILT
...
COMPARISON_DASHBOARD_OUT_OF_SCOPE
VISIBLE_UI_CHECKPOINT_NO
PHASE_11D_H36_DRAFT_PREPARATION_TYPES_READY
R1H5_DEFINED_BUT_NOT_TRIGGERED

## Fase 11D-H32 · Controlled Rename UI
Phase: Fase 11D-H32 · Controlled Rename UI

Summary:
- Built a controlled edit view (`ControlledRenameReview.tsx`) for renaming visible labels (dimensions and questions).
- Integrated the view into `ConversationalImportWorkspace.tsx` under a new view mode `"controlled_rename"`.
- Wired the "Ajustar etiquetas" button into the structure review chat message generated by `demoFixtureStructureReviewMapper.ts`.
- Local changes are stored in memory (`globalOverlayState`) and visually represented in the chat message with "[Ajuste local]" tags.
- Added strict inline validations using pure validation logic from `overlay-editing`.
- Confirmed zero mutations to the `qsClimaFixture` and zero storage/persistence connections.

Commit SHA: PENDING_COMMIT

PHASE_11D_H32_CONTROLLED_RENAME_UI_COMPLETE
CONTROLLED_RENAME_UI_VISIBLE
ADJUST_LABELS_ACTION_VISIBLE
EDIT_MODE_CAN_BE_ENTERED
DIMENSION_LABEL_RENAME_INPUTS_VISIBLE
QUESTION_LABEL_RENAME_INPUTS_VISIBLE
ONLY_VISIBLE_LABELS_EDITABLE
LOCAL_OVERLAY_STATE_USED
LOCAL_ADJUSTMENT_BADGE_VISIBLE
LOCAL_CHANGES_COUNTER_VISIBLE
SAVE_LOCAL_ADJUSTMENTS_AVAILABLE
CANCEL_LOCAL_ADJUSTMENTS_AVAILABLE
SOURCE_FIXTURE_NOT_MUTATED
NO_MOVE_QUESTION_UI
NO_DRAG_AND_DROP
NO_PERSISTENCE
NO_EDITOR_FOR_METRICS_DEMOGRAPHICS_SEGMENTS
AUTO_DETECTION_FLOW_PRESERVED
...
COMPARISON_DASHBOARD_OUT_OF_SCOPE
VISIBLE_UI_CHECKPOINT_YES
PHASE_11D_H33_CONTROLLED_RENAME_VISUAL_QA_READY
R1H5_DEFINED_BUT_NOT_TRIGGERED
...
- PHASE_11D_H38_DRAFT_PREVIEW_UI_COMPLETE
- DRAFT_PREVIEW_ACTION_VISIBLE
- DRAFT_READINESS_PREVIEW_VISIBLE
- DRAFT_READINESS_MAPPER_USED
- DRAFT_READINESS_STATUS_VISIBLE
- DRAFT_SUMMARY_COUNTS_VISIBLE
- DRAFT_BLOCKERS_VISIBLE_WHEN_PRESENT
- DRAFT_WARNINGS_VISIBLE_WHEN_PRESENT
- DRAFT_PRIVACY_BOUNDARY_VISIBLE
- LOCAL_OVERLAY_ADJUSTMENTS_COUNT_VISIBLE
- PREPARE_DRAFT_BUTTON_DISABLED
- PREPARE_DRAFT_BUTTON_HAS_NO_IMPORT_HANDLER
...
- NO_DRAFT_READINESS_MAPPER_CHANGES
- AUTO_DETECTION_FLOW_PRESERVED
- CONTROLLED_RENAME_UI_PRESERVED
- READY_FOR_COMPARISON_OUTPUT_DISABLED
- COMPARISON_DASHBOARD_OUT_OF_SCOPE
- VISIBLE_UI_CHECKPOINT_YES
...
## Fase 11D-H43-H2 · General Configuration UX Corrections
- PHASE_11D_H43_H2_GENERAL_CONFIGURATION_UX_CORRECTIONS_COMPLETE
- GENERAL_CONFIGURATION_IS_7_STEPS
- VISIBILITY_OPTIONS_LIMITED_TO_ANONIMO_PUBLICO
- PRIVATE_VISIBILITY_OPTION_REMOVED
- PUBLICO_LABEL_USED_CONSISTENTLY
- SURVEY_END_DATE_STEP_VISIBLE
- SURVEY_END_DATE_INFERRED_FROM_SURVEY_NAME
- Q1_Q2_Q3_Q4_DATE_INFERENCE_SUPPORTED
- S1_S2_H1_H2_DATE_INFERENCE_SUPPORTED
- YEAR_ONLY_DATE_INFERENCE_SUPPORTED
- USER_CAN_CONFIRM_INFERRED_END_DATE
- USER_CAN_OVERRIDE_END_DATE_BY_CHAT
- INVALID_END_DATE_REPROMPTS
- GENERAL_CONFIGURATION_SUMMARY_INCLUDES_END_DATE
- THINKING_INDICATOR_BEFORE_EVERY_AGENT_STEP
- NO_DIRECT_AGENT_REPLY_WITHOUT_THINKING_STATE
- NO_STUCK_LAST_ASSISTANT_STATE
- STRUCTURE_MATCH_REVIEW_STILL_STARTS_AFTER_GENERAL_CONFIGURATION
- ALL_ACTIONS_BY_CHAT_TEXT_ONLY
- NO_IMPORT_EXECUTION
- NO_SANDBOX_IMPORT_RUNTIME
- NO_RESULT_LINK_CREATED
...
- READY_FOR_COMPARISON_OUTPUT_DISABLED
- VISIBLE_UI_CHECKPOINT_YES
- PHASE_11D_H45_AMBIGUITY_RESOLUTION_FLOW_READY
- R1H5_DEFINED_BUT_NOT_TRIGGERED
- SURVEY_TYPE_OPTIONS_LIMITED_TO_CLIMA_CULTURA_NPS
- ENGAGEMENT_OPTION_REMOVED_FROM_SURVEY_TYPE
- MIXED_OPTION_REMOVED_FROM_SURVEY_TYPE
- NPS_LABEL_USED_CONSISTENTLY

### Phase 11D-H44-H4 · Chat Foundation Thinking Behavior
- Created `chatFoundationThinkingBehavior.ts` containing pure functions to handle thinking policy, evaluation, and message generation.
- Exported from `chat-foundation/index.ts`.
...
CHAT_FOUNDATION_VISUAL_PLAYGROUND_CREATED
PLAYGROUND_VISIBLE_IN_LOCALHOST
PLAYGROUND_IS_ISOLATED
PLAYGROUND_LABELS_INTERNAL_SANDBOX
DEMO_MESSAGES_CREATED
DEMO_MESSAGES_ARE_TYPED
THINKING_MESSAGE_RENDERED_IN_PLAYGROUND
COMMON_POLICY_MESSAGES_RENDERED_IN_PLAYGROUND
SAFETY_POLICY_MESSAGES_RENDERED_IN_PLAYGROUND
PLAIN_TEXT_MESSAGE_RENDERED_IN_PLAYGROUND
STRUCTURED_MESSAGE_RENDERED_IN_PLAYGROUND
CONFIRMATION_MESSAGE_RENDERED_IN_PLAYGROUND
WARNING_MESSAGE_RENDERED_IN_PLAYGROUND
ERROR_MESSAGE_RENDERED_IN_PLAYGROUND
SAFE_DETAILS_MESSAGE_RENDERED_IN_PLAYGROUND
HANDOFF_MESSAGE_RENDERED_IN_PLAYGROUND
ASSISTANT_AVATAR_VISIBLE_IN_PLAYGROUND
ASSISTANT_ICON_VISIBLE_IN_PLAYGROUND
NO_INPUT_INTERACTION_ADDED
NO_FLOW_MIGRATION
NO_CONVERSATIONAL_IMPORT_WORKSPACE_CHANGES
NO_IMPORT_EXECUTION
NO_SANDBOX_IMPORT_RUNTIME
...
READY_FOR_COMPARISON_OUTPUT_DISABLED
VISIBLE_UI_CHECKPOINT_YES
PHASE_11D_H44_H7_CHAT_FOUNDATION_VISUAL_QA_READY
R1H5_DEFINED_BUT_NOT_TRIGGERED
...
## PHASE_11D_H47_H1_REMOVE_UNAUTHORIZED_AMBIGUITY_ROOT_TEST_COMPLETE
UNAUTHORIZED_ROOT_TEST_REMOVED
AUTHORIZED_SRC_TEST_STILL_PRESENT
AMBIGUITY_DETECTION_TEST_COVERAGE_PRESERVED
NO_DETECTION_LOGIC_CHANGES
NO_RUNTIME_INTEGRATION
NO_WORKSPACE_CHANGES
NO_CHAT_FOUNDATION_CHANGES
...
THINKING_STATE_SOURCE = simulateChatFlow batch gaps
