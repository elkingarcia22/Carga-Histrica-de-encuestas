/**
 * Phase 11D-H47 · Ambiguity Detection Mapper — Unit Tests (Vitest runner location)
 *
 * This file mirrors the canonical test at:
 * src/features/historical-import/conversational-import/ambiguity-resolution/__tests__/ambiguityDetectionMapper.test.ts
 *
 * It is placed here so that the vitest test runner (configured with
 * include: ['tests/**\/*.test.ts']) can discover and execute it.
 *
 * Rules:
...
} from '../../../../src/features/historical-import/conversational-import/ambiguity-resolution/ambiguityDetectionMapper';
...
describe('detectHistoricalImportAmbiguities', () => {
  ...
  it('never exposes PII or raw rows — resolvedAmbiguities is always empty in H47', () => {
    ...
    expect(explanation).not.toMatch(/\d{8,}/);
  ...
  // Additional: pendingAmbiguities ordering is deterministic
